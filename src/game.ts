...
//  the noise pulls a hunter off his round to go and look. Reach the green tile.
...
import { load, LEVELS, gems, spawn, exit, par, wx, wz, ti, tj, solidBox, COLS, ROWS, TILE } from './level';
...
// drains along with everything else when the hunters have taken enough.
...
low.style.cssText = 'opacity:.7;font-size:13px;white-space:pre';
ui.append(top, mid, low);
document.body.appendChild(ui);

const two = (n: number) => (n < 10 ? '0' : '') + n;
const secs = (s: number) => s.toFixed(1);

// --- state ------------------------------------------------------------------------------
type Phase = 'title' | 'intro' | 'play' | 'won' | 'end';
...
function reset(caught: boolean) {
  ...
  ringAge = -1;
  clock = 0;
  // The hunters restart their rounds too, so every attempt at a level plays out the same
...
function hud() {
  ...
  if (phase === 'title') {
    top.textContent = '';
    mid.textContent = `${NAME}\n\n`;
    low.textContent = 'SPACE · START      ARROWS / WASD · MOVE      SPACE · FART      M · MUTE';
  } else if (phase === 'intro') {
    top.textContent = `MISSION ${n}`;
    mid.textContent = phaseT > 0.4 ? `MISSION ${n}` : 'START';
    low.textContent = `TARGET ${par} S`;
  } else if (phase === 'play') {
    top.textContent = `MISSION ${n}   ${secs(clock)}`;
    mid.textContent = caughtT > 0 ? 'SPOTTED' : '';
    low.textContent = isMuted() ? 'MUTED' : '';
  } else if (phase === 'won') {
    const b = best(level);
    top.textContent = `MISSION ${n}   ${secs(clock)}`;
    mid.textContent = `MISSION COMPLETE\n${secs(clock)} S · TARGET ${par} S · ${clock <= par ? 'RANK 1' : clock <= par * 1.5 ? 'RANK 2' : 'RANK 3'}`;
    low.textContent = b ? `BEST ${secs(b)} S` : '';
  } else {
    top.textContent = '';
    mid.textContent = 'ALL MISSIONS COMPLETE';
    low.textContent = 'SPACE · AGAIN';
  }
}
...
export function update(dt: number) {
  ...
  if (phase === 'title') {
    if (pressed.has('Space') || pressed.has('Enter')) begin(0);
    return hud();
  }
  if (phase === 'end') {
    if (pressed.has('Space')) { grey = 0; begin(0); }
    return hud();
  }
  if (phase === 'intro') {
  ...
  if (phase === 'won') {
    if ((phaseT -= dt) <= 0) {
      if (level + 1 < LEVELS.length) begin(level + 1);
      else phase = 'end';
    }
    return hud();
  }
  ...
  if (Hunter.sees(pi, pj)) {
    ...
    caughtT = 0.9;
    for (const h of Hunter.seers(pi, pj)) { h.mark = 2; h.markT = 0.9; }
  ...
  if (pi === exit.i && pj === exit.j) {
    phase = 'won';
    phaseT = 2.2;
    const b = best(level);
    if (!b || clock < b) localStorage.setItem(STORE + level, secs(clock));
...
function drawHunter(h: Hunter.Hunter) {
  place(bodyM, h.x, 0, h.z, 0, Math.PI / 2 - h.yaw, Hunter.SCALE);
  ...
  if (h.mark) {
    ...
    place(tmpM, h.x, 2.55 + bounce, h.z, cam.pitch, 0, 1.7);
...
export function draw() {
  ...
  // reach of a hunter's hearing so the player learns how far a noise carries.
  setBlend(true);
  for (const g of gas) {
    const k = g.age / 1.1;
